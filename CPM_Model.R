...
#------------------------------------------------------------#
#----------CPM Utility Functions for Running Model-----------#
#------------------------------------------------------------#
...
train_cpm <- function(train_mat, train_behav, num_nodes, p_thresh = 0.05, mode = "linear", covariates = NULL) {
  ...
  # Compute correlations (partial correlations for covariates if given)
  if (!is.null(cov_mat)) {
    # partial correlation 
  ...
  # Sum edges for positive and negative
...
run_cpm_pipeline <- function(json_file, mat_file_path, mat_file_name, output_dir, covariates = NULL) {
  ...
  # Load .mat file (using the utility function)
